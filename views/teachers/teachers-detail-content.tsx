...
import { TeachersEditProfile } from './components/agent-edit-profile'
import { deleteTeachers, editTeachersProfile, getTeachersById, updateTeachers } from './teachers-service'

type Checked = DropdownMenuCheckboxItemProps['checked']

export function TeachersDetailPageContent() {
  const { id } = useParams();
  ...
  useEffect(() => {
    getTeachersById(`${id}`).then((response) => {
      setTeachers(response);
    })
  }, []);

  const handleDeleteTeachers = async (id: string) => await deleteTeachers(id);
  ...
  const onSubmit = (data: TeachersInput) => {
    ...
    updateTeachers(`${id}`, payload).then((response) => {
      setLoading(true)
      setTeachers(response)
  ...
  const handleProfileUpdate = (updatedData: { name: string, email: string, }) => {
    setLoading(true)
    editTeachersProfile(`${id}`, updatedData).then((response) => {
        ...
        <Card className="flex items-start justify-between overflow-hidden dark:bg-neutral-900 p-6">
          <div className="flex items-center gap-6">
            <Avatar className="w-24 h-24 bg-esporte-neutral-lighter shadow-md flex justify-center items-center dark:bg-neutral-950">
              ...
              <AvatarFallback>{getInitials(`${teachers?.name}`)}</AvatarFallback>
            </Avatar>
            {/* <div className="flex flex-col gap-2">
              <h1 className="text-xl font-semibold">{teachers?.name}</h1>
              <div className="flex items-center gap-2 text-sm text-neutral-500">
                <Hash className="h-4 w-4" />
                <span>Matrícula: {teachers?.email}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-neutral-500">
                <School className="h-4 w-4" />
                <span>Escola: {teachers?.name}</span>
              </div>
              <div className="flex flex-wrap gap-2 mt-1">
                {teachers?.name && teachers?.sports.map((sport) => (
                ...
                {(!teachers?.sports || teachers?.sports.length === 0) && (
          ...
          <div className="flex gap-3 self-end md:self-auto">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem className="text-red-600 cursor-pointer" onClick={() => handleDeleteTeachers(`${teachers?.id}`)}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{teachers?.name}</p>
                    </div>
                  </div>
            ...
            <TabsContent value="activities" className="mt-0 w-full">
              <Card className="p-6 dark:bg-neutral-900">
                ...
                {/* <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {teachers?.name && teachers?.name.length > 0 ? (
                    teachers?.sponamerts.map((sport, index: number) => (
